...
Options:
 ...
 if neither selected the program chooses the last values it gets from the file

 """

const doc = docTmpl % [commitHash, currentDate]

proc readandcut*[T](h5f: var H5FileObj, runNumber: int, chip: int): seq[T] =
...
proc plotcuts*[T](cuts: seq[T]) =
  ...
  let
    d = Trace[float](`type`: PlotType.Histogram,
                    bins: (0.0, 60.0), binSize: 0.1 ) ##somehow change bin and binsize in respect to the data
  d.xs = cuts
  ...
  if runNum == "nil":
    for grp in groups:
      if grp.match(runRegex, run) == true:
        ...
        cuts = readandcut[float](h5f, runNumint, chipNumint)
        #echo cuts
        #plotcuts(cuts)
  else:
