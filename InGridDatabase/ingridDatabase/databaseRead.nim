...
proc getScurveSeq*(chipName: string, runPeriod: string): SCurveSeq =
...
proc getTotCalibParameters*(chipGrp: H5Group, chipName: string, run: int):
  ...
  let runPeriod = findRunPeriodFor(chipName, run, chipGroup = chipGrp)
  result = (getTotCalibParameters(chipName, runPeriod), runPeriod)

proc getCalibVsGasGainFactors*(chipGroup: H5Group, chipName: string,
                               run: int,
                               suffix = ""): tuple[b, m: float] =
  ## returns the fit parameters (no errors) for the given chip
  ## of the calibration of Fe charge spectrum vs gas gain
  withDatabase:
    h5f.visitFile()
    let runPeriod = h5f.findRunPeriodFor(chipName, run, chipGroup = chipGroup)
...
proc inDatabase*(chipName: string, run: int): bool =
  ...
  withDatabase:
    h5f.visitFile()
    # constraints don't matter to make sure the chip exists at all
...
proc initCalibInfo*(h5f: H5File,
  ...
  let runGroupName = basePath & $runNumber
  let chipGroupName = runGroupName / &"chip_{chipNumber}"
  let chipGrp = h5f[chipGroupName.grp_str]
  let ((a, b, c, t), runPeriod) = chipGrp.getTotCalibParameters(chipName, runNumber)
  echo "Getting parameters for ", runNumber, " of hip ", chipName, " a ", a, " b ", b, " c ", c, " t ", t

  # get factors for charge / gas gain fit
  let (bL, mL) = chipGrp.getCalibVsGasGainFactors(chipName, runNumber,
                                                  suffix = $gcIndividualFits)
  # now compute gas gain to use here by computing mean of all gas gain slices in this run (most sensible)

  let gain = h5f[chipGrp.name / "gasGainSlices", GasGainIntervalResult].mapIt(it.G).mean

  # check if run is a CDL run
  let runGrp = h5f[runGroupName.grp_str]
  ...
  if isCdl:
    ...
    doAssert "fit_μ" in chipGrp.attrs, "No fit_μ attribute in " & $chipGrp
    let peak_μ = chipGrp.attrs["fit_μ", float]
    let energy = toXrayLineEnergy(tfKind)
    calibFactor = energy / peak_μ
    # and now read the width of charge distribution
    doAssert "fit_σ" in chipGrp.attrs, "No fit_σ attribute in " & $chipGrp
    peak_σ = chipGrp.attrs["fit_σ", float]
